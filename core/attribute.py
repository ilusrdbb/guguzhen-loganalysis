...
def get_attribute_map(battle_log_str, talent_list, enemy_level):
    ...
    # 获取属性 物穿
    attribute_pth = get_pth(gear_list, talent_list)
    # 获取属性 魔穿
    attribute_mth = get_mth(gear_list, talent_list)
    # 获取属性 暴穿
    attribute_cth = get_cth(gear_list)
    # 推测加点 力量
    cal_str = get_str(attribute_patk, talent_list, gear_list, enemy_level)
    # 推测加点 智力
    cal_int = get_int(attribute_matk, talent_list, gear_list, enemy_level)
    # 推测加点 敏捷
    cal_agi = get_agi(attribute_spd, gear_list)
    # 获取属性 附加物穿
    _attribute_pth = _get_pth(gear_list, talent_list, attribute_pth, cal_str, enemy_level)
    # 获取属性 附加魔穿
    _attribute_mth = _get_mth(gear_list, talent_list, attribute_mth, cal_int, enemy_level)
    # 获取属性 物理减伤
    attribute_pdec = get_pdec(gear_list, talent_list, enemy_level)
    # 获取属性 魔法减伤
    attribute_mdec = get_mdec(gear_list, talent_list, enemy_level)
    # 获取属性 回血比例
    attribute_hpr = get_hpr(cal_str)
    # 获取属性 回盾比例
    attribute_sldr = get_sldr(cal_int, gear_list)
    # 获取属性 附加回血
    _attribute_hpr = _get_hpr(gear_list)
    # 获取属性 附加回盾
    _attribute_sldr = _get_sldr(gear_list)
    # 获取属性 暴击
    attribute_crt = get_crt(gear_list, cal_agi)
    # 获取属性 技能
    attribute_skl = get_skl(gear_list, cal_int)
    # 获取属性 反弹
    attribute_rfl = get_rfl(gear_list, talent_list)
    # 获取属性 吸血
    attribute_lch = get_lch(gear_list, talent_list)
    return {
        'hp': attribute_hp,
        'sld': attribute_sld,
...
# 获取属性 吸血
def get_lch(gear_list, talent_list):
    lch = gear.get_sum_point(gear_list, 'LCH')
...
# 获取属性 技能
def get_rfl(gear_list, talent_list):
    rfl = gear.get_sum_point(gear_list, 'RFL')
...
# 获取属性 技能
def get_skl(gear_list, cal_int):
    skl = gear.get_sum_point(gear_list, '_SKL')
    skl = skl + cal_int * INT_MU_SKL
    return skl


# 获取属性 暴击
def get_crt(gear_list, cal_agi):
    crt = gear.get_sum_point(gear_list, '_CRT')
    crt = crt + cal_agi * AGI_MU_CRT
    return crt


# 获取属性 附加回盾
def _get_sldr(gear_list):
    return gear.get_sum_point(gear_list, '_SLDR')


# 获取属性 附加回血
def _get_hpr(gear_list):
    return gear.get_sum_point(gear_list, '_HPR')


# 获取属性 回盾比例
def get_sldr(cal_int, gear_list):
    sldr = int(cal_int / INT_DIV_SLDR)
    sldr = sldr + gear.get_sum_point(gear_list, 'SLDR')
    return sldr
...
# 获取属性 魔法减伤
def get_mdec(gear_list, talent_list, enemy_level):
    mdec = gear.get_sum_point(gear_list, '_MDEC')
...
# 获取属性 物理减伤
def get_pdec(gear_list, talent_list, enemy_level):
    pdec = gear.get_sum_point(gear_list, '_PDEC')
    if 'SHI' in talent_list:
...
# 获取属性 附加魔穿
def _get_mth(gear_list, talent_list, attribute_mth, cal_int, enemy_level):
    _mth = gear.get_sum_point(gear_list, '_MTH')
...
# 获取属性 附加物穿
def _get_pth(gear_list, talent_list, attribute_pth, cal_str, enemy_level):
    _pth = gear.get_sum_point(gear_list, '_PTH')
...
# 推测加点 敏捷
def get_agi(attribute_spd, gear_list):
    spd = gear.get_sum_point(gear_list, 'SPD')
    _spd = gear.get_sum_point(gear_list, '_SPD')
...
# 推测加点 力量
def get_str(attribute_patk, talent_list, gear_list, enemy_level):
    patk = gear.get_sum_point(gear_list, 'PATK')
    _patk = gear.get_sum_point(gear_list, '_PATK')
...
# 推测加点 智力
def get_int(attribute_matk, talent_list, gear_list, enemy_level):
    matk = gear.get_sum_point(gear_list, 'MATK')
    _matk = gear.get_sum_point(gear_list, '_MATK')
...
# 获取属性 暴穿
def get_cth(gear_list):
    return gear.get_sum_point(gear_list, 'CTH')


# 获取属性 物穿
def get_pth(gear_list, talent_list):
    pth = gear.get_sum_point(gear_list, 'PTH')
    if 'BI' in talent_list:
...
# 获取属性 魔穿
def get_mth(gear_list, talent_list):
    mth = gear.get_sum_point(gear_list, 'MTH')
